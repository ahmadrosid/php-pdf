...
cargo_build:
	...
	if [[ -f ./target/$CARGO_MODE_DIR/libphp_pdf.dylib ]] ; then \\
		cp ./target/$CARGO_MODE_DIR/libphp_pdf.dylib ./modules/libphp_pdf.so ; fi
	if [[ -f ./target/$CARGO_MODE_DIR/libphp_pdf.so ]] ; then \\
		cp ./target/$CARGO_MODE_DIR/libphp_pdf.so ./modules/libphp_pdf.so ; fi
